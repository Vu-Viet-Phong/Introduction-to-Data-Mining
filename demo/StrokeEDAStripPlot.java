...
public class StrokeEDAStripPlot {
    ...
    public static void main(String[] args) {
        ...
        // Create a strip plot of avg glucose level vs. stroke status
        CategoryPoints points = new CategoryPoints();
        points.setX(Arrays.asList("0", "1"));
        points.setY(strokeData.stream().map(row -> Double.parseDouble(row.get("avg_glucose_level")))
                .collect(Collectors.toList()));

        CategoryPlot plot = new CategoryPlot();
        plot.add(points);
        plot.setTitle("Avg Glucose Level vs. Stroke Status");
        plot.setXLabel("Stroke");
        plot.setYLabel("Avg Glucose Level");
